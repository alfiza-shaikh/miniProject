...
import pytesseract
from detection_model.core.config import cfg
import re
...
def recognize_plate(img, coords):
    ...
    box = img[max(int(ymin)-5,0):min(int(ymax)+5,len(img)), max(int(xmin)-5,0):min(int(xmax)+5,len(img[0]))]
    ...
    for cnt in sorted_contours:
        ...
        # cnt_img = cv2.drawContours(dilation, [cnt], -1,(0, 255, 0), 2)
        # cv2_imshow(cnt_img)
        # grab character region of image
        #roi = dilation[y-5:y+h+5, x-5:x+w+5]
        roi = dilation[max(y-5, 0): min(y+h+5, y+height), max(x-5, 0):min(x+w+5, x+width)]
...
def load_config( class_path):
    STRIDES = np.array(cfg.YOLO.STRIDES)
    ANCHORS = get_anchors(cfg.YOLO.ANCHORS)
    XYSCALE = cfg.YOLO.XYSCALE 
    NUM_CLASS = len(read_class_names(class_path))
        ...
        if class_name not in allowed_classes:
            ...
            if read_plate:
                ...
                if plate_number != None:
                    ...
                    output_plate_number = plate_number
            ...
            if counted_classes != None:
                ...
                for key, value in counted_classes.items():
                    ...
                    offset += height_ratio
    return image, output_plate_number
