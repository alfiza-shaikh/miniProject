...
import pytesseract
from detection_model.core.utils import read_class_names
...
def crop_objects(img, data, path, allowed_classes):
    ...
    for i in range(num_objects):
        ...
        if class_name in allowed_classes:
            ...
            cropped_img = img[max(int(ymin)-5,0):min(int(ymax)+5,len(img)), max(int(xmin)-5,0):min(int(xmax)+5,len(img[0]))]
            # construct image name and join it to path for saving crop properly
            #img_name = class_name + '_' + str(counts[class_name]) + '.png'
            #img_path = os.path.join(path, img_name )
            # save image
            #cv2.imwrite(img_path, cropped_img)
            cropped_img_list.append([cropped_img, class_name])
        else:
            continue
